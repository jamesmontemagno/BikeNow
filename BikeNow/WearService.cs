...
namespace BikeNow
{
	...
	{
		const string SearchStationPath = "/bikenow/SearchNearestStations";
		...
		async void HandleMessage (IMessageEvent message)
		{
			try {
				...
				try {
					...
					var stations = Pronto.Instance.LastStations;
					if (stations == null)
						stations = await Pronto.Instance.GetStations ();

					var currentPoint = new GeoPoint {
						Lat = lastLocation.Latitude,
						Lon = lastLocation.Longitude
					};
					var nearestStations = Pronto.GetStationsAround (stations, currentPoint);
