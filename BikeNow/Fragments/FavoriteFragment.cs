...
namespace BikeNow
{
	public class FavoriteFragment : Android.Support.V4.App.ListFragment, IProntoSection
	{
	...
	class FavoriteAdapter : BaseAdapter
	{
		...
		public async void SetStationIds (HashSet<int> ids)
		{
			while (true) {
				try {
					var stationList = await Pronto.Instance.GetStations ();
					stations = stationList
						.Where (s => ids.Contains (s.Id))
						.OrderBy (s => s.Name)
						.ToList ();
				} catch (Exception e) {
					e.Data ["method"] = "SetStationIds";
		...
		public override View GetView (int position, View convertView, ViewGroup parent)
		{
			...
			string secondPart;
			stationName.Text = station.Street;
			secondStationName.Text = station.Name;
